  ...
  ionViewDidLoad() {
    ...
    setTimeout(() => {
      jQuery('.book_shlef_list').css({
        minHeight: jQuery('#scroller').parent().height()
      });
      this.myScroll.refresh();
    }, 1000)
  }
        ...
        if (success.code == 0) {
          if (parseInt(success.data.version_code) > this.service.version_code) {
            if (this.service.platformName == 'ios') {
              this.service.dialogs.alert('你有一个新的版本需要更新!<'+ success.data.version_name +'>', '提示', '确定').then(res => {
              ...
              this.service.fileTransfer.download(url, targetPath, true).then(success => {
                load.dismiss();
                this.service.opener.open(targetPath, 'application/vnd.android.package-archive').then(sss => {
